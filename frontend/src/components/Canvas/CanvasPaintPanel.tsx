...
function CanvasPaintPanel(props: CanvasPaintPanelProps): JSX.Element {
  ...
  return (
    <div className={`canvas-paint-panel ${isExpanded ? "is-expanded" : "is-collapsed"} ${props.isDocked ? "is-docked" : ""}`}>
      <div className={`canvas-paint-expandable ${isExpanded ? "is-open" : "is-closed"}`} aria-hidden={!isExpanded}>
        <div className="canvas-paint-expandable-inner">
          <div className="canvas-paint-popup-header">
            <div className="canvas-paint-panel-heading">
              {props.isDocked && <span className="canvas-paint-panel-title">{CANVAS_COPY.status.selectedColor}</span>}
              <button
                type="button"
      ...
      <button
        type="button"
        className={`canvas-paint-launch-button is-${props.placeState} ${props.hasPlaceError ? "is-error" : ""} ${isExpanded ? "is-active" : ""}`}
        onClick={isExpanded ? props.onPlace : props.onToggleExpanded}
        disabled={isExpanded ? props.isPlaceDisabled : false}
        aria-expanded={isExpanded}
        aria-pressed={isExpanded}
        aria-busy={isExpanded && props.placeState === "loading"}
      >
        {isExpanded && props.placeState === "loading" ? (
          <>
