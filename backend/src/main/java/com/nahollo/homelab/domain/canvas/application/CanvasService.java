package com.nahollo.homelab.domain.canvas.application;
...
public class CanvasService {
	...
	public CanvasStateResponse readCurrent() {
		...
		int placedCount = countEvents(season.id());
		long latestEventId = latestEventId(season.id());
	...
	public CanvasUpdatesResponse readUpdates(long sinceEventId, int limit) {
		CanvasSeasonRecord season = ensureCurrentSeason();
		int normalizedLimit = Math.max(1, Math.min(500, limit));

		List<CanvasPixelUpdate> updates = jdbcTemplate.query(
			"""
		...
		return new CanvasUpdatesResponse(latestEventId(season.id()), updates);
	}
	...
	public CanvasHistoryDetailResponse readHistoryDetail(String seasonCode) {
		...
		if (season == null) {
			throw new IllegalArgumentException("?대떦 ?쒖쫵??李얠쓣 ???놁뒿?덈떎.");
		}
	...
	private long latestEventId(UUID seasonId) {
	...
	private CanvasPixelUpdate mapPixelUpdate(ResultSet resultSet, int rowNum) throws SQLException {
		return new CanvasPixelUpdate(
			...
			normalizeNickname(resultSet.getString("nickname")),
			readInstant(resultSet, "placed_at"),
	...
	private String hashIp(String ipAddress) {
		try {
			...
			throw new IllegalStateException("SHA-256 ?댁떆瑜??ъ슜?????놁뒿?덈떎.", exception);
		}
	}
